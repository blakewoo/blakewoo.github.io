...
이 유도 방식에는 크게 두 개가 있는데, 최좌단(leftmost) 유도, 최우단(rightmost) 유도가 있다.   
각 문장형태에서 가장 왼쪽 논터미널이 선택되어 대체되면 최좌단 유도, 반대로 가장 오른쪽 논터미널이 선택되면 최우단 유도라고 말한다.   

어떤 생성규칙이 아래와 같이 있다고 해보자.

```
E -> E + E | E * E | - E | (E) | id
```

아래는 각 최좌단, 최우단 유도에 대한 간단한 예시이다.
...
중간 까지는 동일하지만 뒤에서 두 번째부터 바뀌는 것을 알 수 있다.
