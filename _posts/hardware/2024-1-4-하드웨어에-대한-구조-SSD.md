...
FTL은 기본적으로는 페이지 단위로 Mapping Table을 구성하는 것이다.
...
페이지 단위로 매핑을 하는 것이다. 가령 1TB의 SSD가 있다고 가정해보자
...
말 그대로 블록 단위로 매핑을 하는 방식이다. 이전 사례와 같이 1TB의 SSD가 있다고 할때
...
DRAM 영역을 Data 영역과 Log 영역으로 분리한 뒤 Data 영역은 Block Mapping 방식으로, Log 영역은 Page Mapping으로
운영하는 것이다.

Log 영역은 Page Mapping이 되어있는데 이 Log 영역에 free 영역이 없다면 Merging 후 Data 영역, 즉 Block 영역에
기재한다.
