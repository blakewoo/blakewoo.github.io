...
이 Total Ordering에는 두 가지 방식이 있다.

#### A. Total Ordering(FIFO)
모든 Receiver에서 정렬한 순서가 FIFO 방식과 동일함

#### B. Total Ordering(Causal)
모든 Receiver에서 정렬한 순서가 Causal Ordering 방식과 동일함
...
### 2) 성능 지표
- Message per CS Entry
- Entry Delay
- Sychronization Delay
- Fairness
- Scalability
- Fault Tolerance / Robustness

### 3) 알고리즘 1 : 중앙화(Centralized)
모든 Node가 임계구역에 접근하고 싶을 때 Lock manager에게 요청 후 lock을 받아 접근하는 방식.   
Lock manager는 각 Node에서 요청을 수신하면 FIFO Queue에 넣고 순차적으로 Lock을 제공한다.

### 4) 알고리즘 2 : 토큰 기반(Token-Based, Token Ring)
모든 노드가 Ring과 같이 연결되어 Ring을 순환하여 Token이 전달되고, 프로세스는 Token을 갖고 있을 때만 임계구역에 접근 가능한 방식.

### 5) 알고리즘 3 : 허가 기반(Permission-Based, Ricart-Agrawala)
임계구역에 접근하고자하는 Node는 다른 모든 노드에게 임계구역에게 접근해도 되는지 확인 요청을 보냄, 요청을 받은 Node는 자신이 임계구역에
접근하지 않고 있을 경우 요청한 Node에게 접근 가능하다고 요청을 보낸다.

만약 노드 A가 다른 노드들에게 임계구역 접근 요청을 하고 노드 B가 A의 요청을 받은 시점에서 B 역시 임계구역을 접근하고자 한다면   
A의 요청에 담긴 Lamport Clock으로 이루어진 logical clock과 B의 현재 logical clock을 비교하여 작은 쪽이 먼저 접근하도록 해주며
만약 logical clock이 동일하다면 A와 B에게 고유하게 할당된 ID 값을 비교하여 ID값이 작은 Node 쪽에게 먼저 접근권을 전달한다.
