...
- static   
  미리 for 문의 iteration 돌 개수대로 나누는 것이다. 다만, static 뒤에 붙는 chunksize에 따라 각 Thread에 할당하는 양이 달라진다. chunksize를 생략하면
  loop count / Thread 개수로 나눠서 할당한다.
  schedule (static, 1) 이고 Thead가 3개로 0,1,2라면 아래와 같이 할당된다.
  - Thread 0 : 0, 3, 6
  - Thread 1 : 1, 4, 7
  - Thread 2 : 2, 5, 8   
...
- guided
  잘게 잘라서 동적으로 분배하면 오버헤드가 크기 때문에 전체 loop개수를 Thread 개수로 나누고, 남은 loop를 다시 Thread 개수로 나눠서 할당하는 방식이다.
  dynamic 보다는 오버헤드가 적다.

- auto   
  그냥 컴파일러에게 맡기는 방식이다.

- runtime   
  구동시 환경변수를 같이 줌으로써 해당 방식대로 구동하는 방식이다.

Chunk size는 기본적으로 양의 정수이다.
