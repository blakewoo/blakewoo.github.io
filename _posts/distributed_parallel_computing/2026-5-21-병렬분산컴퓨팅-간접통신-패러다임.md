...
브로커 (Broker) : 브로커는 구독 테이블(Subscription Table)을 참조하여 들어온 이벤트가 어떤 구독자의 관심사와 일치하는지 평가하고, 해당되는 구독자들에게만 이벤트를 전달한다
...
- 브로커의 수평적 확장(Horizontal Scaling): 고성능 장비 한 대를 쓰는 대신, 저렴한 서버(Commodity hardware)를 여러 대 추가하여 처리 용량을 선형적으로 늘린다.
- 지능형 라우팅(Efficient Routing): 모든 이벤트를 모든 브로커에 뿌리는 플러딩(Flooding) 방식 대신, 구독 정보가 있는 경로로만 이벤트를 보내는 랑데부(Rendezvous) 라우팅 등을 사용하여 불필요한 트래픽을 최소화한다
...
구현은 매우 단순하고 견고하지만, 관심 없는 브로커에게도 데이터가 전송되어 네트워크 대역폭 낭비와 트래픽 오버헤드가 매우 크다.
