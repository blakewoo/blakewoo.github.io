...
### 8) Loader
Loader가 ELF를 파싱하여 map 정의와 program 정의 파악한다.

## 3. 사용 예시
### 1) 예시 1: 프로세스가 파일을 열 때(open) 로그 남기기 (트레이싱)
아이디어는 “파일 open 관련 커널 훅(예: tracepoint/kprobe)에 eBPF 프로그램을 attach해서,
이벤트가 발생할 때마다 PID/커맨드/파일 경로 등을 수집해 유저 공간으로 전달”한다.
