...
struct super_operations {
  struct inode *(*alloc_inode)(struct super_block *sb); // 지정한 수퍼블록안에 새로운 아이노드 개겣를 만들고 초기화한다.
	void (*destroy_inode)(struct inode *); // 지정된 아이노드를 할당 해제한다.
	void (*free_inode)(struct inode *); 

  void (*dirty_inode) (struct inode *, int flags); // vfs가 아이노드가 변경되었을 경우 호출하는 함수다, 저널 갱신용으로 쓴다.
	int (*write_inode) (struct inode *, struct writeback_control *wbc); //지정한 아이노드를 디스크에 기록한다.
	int (*drop_inode) (struct inode *); // vfs가 해당 아이노드에 대한 참조가 모두 사라졌을 경우 호출한다.
	void (*evict_inode) (struct inode *); 
	void (*put_super) (struct super_block *); // vfs가 마운특 ㅏ해제되어 해당 수퍼블록 객체를 제거할때 호출하는 함수다.
	int (*sync_fs)(struct super_block *sb, int wait); // 파일 시스템 메타데이터를 디스크상의 파일 시스템과 동기화한다.
	int (*freeze_super) (struct super_block *, enum freeze_holder who);
	int (*freeze_fs) (struct super_block *);
	int (*thaw_super) (struct super_block *, enum freeze_holder who);
	int (*unfreeze_fs) (struct super_block *);
	int (*statfs) (struct dentry *, struct kstatfs *); // vfs가 파일 시스템 통계정보를 필요로할때 호출된다.
	int (*remount_fs) (struct super_block *, int *, char *); // vfs가 파일 ㅣㅅ스템을 새로운 마운트 옵션으로 다시 마운트할때 호출한다.
