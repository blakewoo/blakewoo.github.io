...
ADA AD102 GPU 기준 각 프로세싱 블록은 아래와 같은 유닛들을 포함하고 있다.   
...
최대 가능한 Warp 수 분의 동시에 구동중인 Warp 수를 기본적으로 구하고 이후
register와 사용하는 shared memory 크기에 따라서 Warp Occupancy가 달라진다.

아래는 compute capability 8.7 (ampere)에서의 스펙 일부이다.
...
사용자가 구동한 커널이 Warp Occupancy가 100%가 되려면 SM당 총 48개의 Warp가 구동되어야한다.

만약 활성 총 Threads가 512개일 경우 512/32로 16개의 warp가 구동된다. 이 경우 Warp occupancy는 16/48로 33.3% 정도이다.
Thread를 블록당 2048개를 구동하면 어떻게 될까? 그래도 1536개만 구동된다. 이는 Ampere 아키텍처의 "Maximum number of resident threads per SM" 스펙에
따른 것이다. 1536개일 경우 1536 / 32로 총 48개의 warp가 구동되는데 이럴 경우 Warp Occupancy가 100%가 된다.
나머지 스레드는 어떻게 되느냐? 그냥 나머지는 기다렸다가 스케줄링 되서 다시 구동된다.
...
동일하게 compute capability 8.7 (ampere)에서의 스펙 일부이다.  
thread block당 32bit 레지스터가 64K개이고, sm당 공유 메모리의 크기가 165KB이다.
커널이 thread 당 64개의 레지스터를 쓴다고 해보자. 64k / 64 = 1024 개의 thread까지 사용할 수 있다.
이 경우 앞서 1536개의 thread를 쓰던 상관없이 1024개의 thread를 사용하게 되며 1024 / 32(Warp size) = 32개의 warp를 사용하게 되므로
66.67%의 Warp Occupancy가 된다.

동일하게 Shared memory를 사용시 thread별로 128 Bytes씩 사용한다고 생각하면
165K / 128 = 1320개이다. 이 경우 1320 / 32 = 41.25로 총 41개의 Warp가 구동된다. 이 경우 Warp Occupancy를 환산하면
41.25 / 48 = 약 86% 정도된다.

이렇듯 Warp Occupancy를 높이려면 최대 구동 가능 Thread 수와 register 개수, SM당 공유 메모리 사용가능 수까지 모두 고려해야한다.
...
#### ※ 분기문이 성능을 떨어뜨리는 이유
크게는 아래와 같이 두 가지로 볼 수 있다.

- 실행 슬롯 낭비   
  특정 경로가 실행되는 동안 해당 조건에 맞지 않는 나머지 스레드들은 유휴(Inactive, Idle) 상태가 되어 하드웨어 자원이 낭비된다.   

- 낮은 하드웨어 활용도   
  발산은 워프 스톨(Stall)을 유발하며, 이는 결국 실행 유닛(Execution Units)의 활용도를 떨어뜨리는 결과로 이어진다.

연산 하나를 1사이클로 처리할 수 있도록 설계되었다고 하면 직렬화되어 분기를 하나씩 처리하기 때문에 분기가 2개인 경우
2사이클, 분기가 3인 경우 3사이클로, 분기 개수에 따라 N $\times$ 사이클이 소요되어 성능이 낮아진다.
