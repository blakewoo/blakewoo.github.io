---
title: Vector DB - ANNs(approximate nearest neighbor search)
author: blakewoo
...
# Vector DB - ANNs(approximate nearest neighbor search)
## 1. 개요
...
제일 정확한 방법은 너무 명료하다. 어떤 것보다 가장 가깝다라는 것은 기본적으로 전체를 확인했다는 것이 전체된 것이다.   
수학적 수식이나 논리적 귀결에 의해 나머지 경우를 배제할 수 있는 경우가 아니라면(이것도 심지어 전처리가 되어있어야하는 경우가 많다,
가령 정렬되어있다던가) 대부분 전체를 확인해야한다.

전체 데이터 갯수가 N개라고 할 때 시간복잡도는 평균적으로 O(N)이 된다. (Quick selection 기준)

그렇다면 제일 빠른 방법은 무엇일까?    
사실 가장 빠르기만 해선 안된다. 막말로 가장 빠른 것만 생각한다면 아무점이나 찍어서 K개만큼 반환 해줄 수도 있다.   
(이게 뭐야 할수도 있지만, 일단 빠르긴하다 ;;)   
때문에 빠른 것과 동시에 어느정도 정확성까지 보장되어야한다. 여기서 시작된 방식이 바로 유사 근접 이웃(ANN)을 찾는(Search) 방법이다.   
...
### 3. 그래프 기반
가장 핫하다. 이는 동일한 재현율(recall rate : 0.8) 기준으로 이 방식이 가장 지연시간이 짧기 때문이다.   
여기서 크게 두 가지로 나뉜다.   
...
### 4. IVF(Inverted File Index) 기반
현업에서는 그래프 방식을 많이 사용한다고 하지만 GPU의 병렬성과 메모리 제한이 클 경우 선택할 수 있는 방식이다.   
일반적으로 그래프 방식에 비해서 Recall이 떨어지는 방식으로 알려진 이 방식은 아래와 같은 파생 타입이 있다.

#### 1) IVF-Flat
가장 기본이 되는 방식이다. IVF 기반 방식은 벡터 공간을 여러 클러스터로 나눈 뒤,
질의 시 모든 벡터를 다 보지 않고 질의와 가까운 몇 개 클러스터만 탐색해서 속도를 높이는 ANN 인덱싱 계열을 말한다.

여기서 클러스터링할때는 흔히들 [K-means](https://blakewoo.github.io/posts/%EA%B8%B0%EA%B3%84%ED%95%99%EC%8A%B5-k-means/) 방식을
많이 사용하며 여기서 나온 Centroid들, 즉 클러스터의 중심점들만 메모리에 상주시켜서 질의가 들어오면 해당 중심점들로만 유사도를 측정 후 
상위 몇 개(nprobe)의 해당 중심점에 대한 클러스터들의 원본 Vector 값을 모두 가져와서 질의와 유사도를 계산하는 방식이다.   

Brute force 방식 대비 계산량이 줄어드는건 확실하겠지만 만약 정답 벡터가 들어있는 클러스터가 초기에 선택되지 않을 경우 정답은 선택되지 않기 때문에
그래프 방식에 비해서 일반적으로 Recall이 떨어진다.

이 방식에서 중요한 것은 전체를 몇 개의 클러스터(nlist)로 나누었냐와 상위 몇 개의 클러스터(nprobe)를 비교할 것인가이다.   
nlist와 nprobe 값의 크기에 따른 이득과 손실은 아래와 같다.

- #### nlist가 작을 경우   
  한 클러스터의 크기가 커져서, TOP-K를 구하는데 시간이 많이 걸리나 정답 Vector가 들어있는 클러스터가 선택될 가능성이 높아짐.
  

- #### nlist가 클 경우   
  한 클러스터의 크기가 작아져서, TOP-K를 구하는데 시간이 적게 걸리나 정답 Vector가 들어있는 클러스터가 선택될 가능성이 낮아짐.
  

- #### nprobe가 작을 경우   
  확인 대상 클러스터 갯수가 적어져서 정답 Vector가 들어있는 클러스터가 선택될 가능성이 낮아지나 연산량이 적어져 TOP-K를 구하는데 시간이 적게 걸림
  

- #### nprobe가 클 경우   
  확인 대상 클러스터 갯수가 많아져서 정답 Vector가 들어있는 클러스터가 선택될 가능성이 높아지나 연산량이 많아져 TOP-K를 구하는데 시간이 많이 걸림

위와 같이 상호보완적인 느낌이 있기 때문에 nlist와 nprobe 크기는 실험을 통해 적절한 선정이 필요하다.

#### 2) IVF-PQ
원래 IVF-Flat 방식에서 클러스터 내의 원본 Vector를 [PQ 양자화](https://blakewoo.github.io/posts/VectorDB-Product-Quantization/) 한 것이다. 
IVF-Flat 방식은 클러스터 중심점만 메모리에 올려둔다고 하더라도 클러스터가 선택되고 나면 해당 클러스터 내에서 질의와 Top-k를 찾기 위해
대상 클러스터 내의 모든 Vector를 메모리에 올려서 질의와 비교해야하는데, 이 Vector 값들도 대규모 데이터 셋의 경우 클러스터로 나누었다고 해도
용량이 클 수 있다. 이를 해결하기 위해 PQ 양자화를 통해 용량을 줄이는 것이다.
