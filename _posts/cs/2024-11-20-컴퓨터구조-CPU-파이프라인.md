...
## 1. CPU가 명령어를 처리하는 대략적인 순서(MIPS 기준)
1. IFetch : 명령어를 갖고 오고 프로그램 카운터에 업데이트하는 과정
2. Dec : 명령어를 해석하고 레지스터에 패치하는 과정
3. Exec : 명령어를 실행하는 과정으로 계산하거나 메모리 주소를 계산하는 과정
4. Mem : 데이터 메모리에서 데이터를 읽거나 쓰는 과정
5. WB : 연산 결과 데이터를 레지스터에 쓰는 과정

이러한 순서는 CPU의 종류와 파이프라인 수준에 따라 달라진다.
